...
import { lobbyType, siteContextType, userType } from '@/types'
...
export default function LobbyView({ initialLobby, token, players }: { initialLobby: lobbyType | null | undefined, token?: string, players: userType[] }) {
                ...
                <div className='flex flex-col absolute top-0 left-0 w-full h-full items-center justify-center backdrop-blur-sm bg-opacity-30 bg-black z-[101]'>
                    <div className='flex flex-col bg-white rounded'>
                        ...
                        {lobby?.lobbyResult == lobbyWaitingForDraft && (
                            <LobbyDraft lobby={lobby} players={players} userId={user?._id.toString()} token={token}/>
                        )}
                    </div>
                </div>
            ): null}
